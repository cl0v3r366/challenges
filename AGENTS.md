...
- **Port completeness:** a green `pwnshop test` is not a complete port. Every module named
  in a dojo needs its own `module.yml`; validate with `tools/dojo/parse-dojo-yml --json`
  and confirm expected level counts. Preserve the old lectures, headers, markdown, challenge
  names/order, and every `DESCRIPTION.md`; if headers are interleaved with levels, put
  lectures, headers, and `type: challenge` entries in one ordered `resources:` list because
  the parser appends `resources` before `challenges`. After moving a module, grep docs,
  README, and AGENTS/CLAUDE files for stale paths.
...
- **Testing integrity:** run `pwnshop test` **only inside `nix develop`** (outside it,
  ...
  runs the real solve and greps the flag; validate private solves with `--require-solved`
  when reviewing/porting because pwnshop tracks "passed" (exit 0) separately from "solved"
  (the real random `/flag` appeared). A dummy flag, local simulator, or direct `/flag` read
  is a fake pass. `tests_public` must exercise real functionality, not just `ls /challenge`.
  Timing/network tests need reliability evidence (looped runs or `--attempts`); tune them
  in the pwnshop container, not only on the host, and wrap fast dojjail/netns startup races
  in a bounded retry. **Never claim tests pass without running them**; qualify a pass with
  its environment.
- **Git/comms:** on an unpushed WIP branch just amend obvious fixes (no permission menus);
  ...
  lands in history. Before committing private tests, verify the staged blobs are encrypted
  (`git crypt status`, or `git cat-file -p :path` starts with `\0GITCRYPT`); an unlocked
  plaintext worktree is not proof the index is safe.
...
If `pwnshop` resolves to a user-local wrapper or import-broken executable inside the dev
shell, retry with the repo-local `./pwnshop` before treating the failure as a challenge bug.
...
- pwnshop renders each `.j2` file with a fresh seeded `random`; to share seeded values
  between a challenge body and its test, call the same config macro first in both files and
  pass `random` into imported macros explicitly.

### Critical Template Details
- Python scripts that need SUID should use shebang: `#!/usr/bin/exec-suid -- /usr/bin/python3 -I`
- The rendered shebang must be physical line 1. Put trimmed Jinja imports/sets before the
  shebang if needed; never put `{%- ... -%}` immediately after a shebang where it can eat
  the newline and merge the shebang into code. Render-check script templates before fan-out.
...
- Public tests verify actual functionality; placeholder `ls /challenge` tests are not enough
- Private tests contain exploitation logic and should emit the real random flag; use
  `pwnshop test --require-solved` to catch fake passes
